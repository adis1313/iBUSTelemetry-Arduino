...
void iBUSTelemetry::setPin(uint8_t pin)
{
  pinMode(pin, INPUT);
...
void iBUSTelemetry::addSensor(uint8_t type){
...
void iBUSTelemetry::setSensorValue(uint8_t adr, int32_t value)
{
...
void iBUSTelemetry::setSensorValueFP(uint8_t adr, float value)
{
  uint8_t sensAdrArr = adr - 1;
...
uint8_t iBUSTelemetry::getSensorSize(uint8_t type)
{
  return (type < 0x80)? 2: 4;
}

uint16_t iBUSTelemetry::gpsStateValues(uint8_t firstVal, uint8_t secondVal)
{
	return (secondVal << 8) | firstVal;
}

void iBUSTelemetry::run()
{
